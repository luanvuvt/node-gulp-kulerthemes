...
if (!cmdTasks[0] || themeDevTasks.indexOf(cmdTasks[0]) != -1) {
	...
	gulp.task('pkg', ['pkg-clean'], function () {
		...
		function createPackage() {
			var resourcePath = ejs.render(themeSettings['resourcePath'], options);
			...
			stream.on('end', function () {
				...
				themeSettings['opencarts'].forEach(function (opencart) {
					...
					opencart['items'].forEach(function (itemPath) {
						itemPath = path.normalize(ejs.render(itemPath, options));
